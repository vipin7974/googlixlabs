...
export const alt = "GooglixLabs — Website Design & Digital Growth Services";

export default function LocationOpengraphImage({ params }: { params: { slug: string } }) {
  const location = getLocationBySlug(params.slug);
  const name = location?.name ?? "Your City";
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <span style={{ fontSize: 28, fontStyle: "italic", color: "#2B5CFF" }}>
            Digital growth for {name} businesses
          </span>
          ...
          <span style={{ fontSize: 22, color: "#77776F", marginTop: 8 }}>
            Free tools · Real quotes · A team that answers the phone
          </span>
        </div>
